...
function makeLibSignalRepository(auth) {
    ...
    return {
        decryptGroupMessage({ group, authorJid, msg }) {
            ...
            // Wrap in transaction so key-store writes are atomic:
            // if decryption fails (Bad MAC), session state is not partially committed.
        ...
        async processSenderKeyDistributionMessage({ item, authorJid }) {
            ...
            await auth.keys.transaction(async () => {
        ...
        async decryptMessage({ jid, type, ciphertext }) {
            ...
            // Wrap in transaction so all session reads/writes within libsignal are atomic.
            // Without this, a Bad MAC mid-decryption can leave the session in a corrupt state.
            return auth.keys.transaction(async () => {
                let result;
                switch (type) {
                    case 'pkmsg':
